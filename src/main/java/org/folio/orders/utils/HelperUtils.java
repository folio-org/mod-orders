...
public class HelperUtils {
   ...
   * @return         the last composed element (Future result)
   */
  ...
  /**
   * The method allows to compose any elements with the same action in sequence and return the accumulated result.
   *
   * @param  list    elements to be executed in sequence
   * @param  method  action that will be executed sequentially based on the number of list items
   * @return         the accumulated result of all composed elements (Future result)
   */
  public static <T, R> Future<List<R>> chainCallAccumulated(List<T> list, Function<T, Future<R>> method) {
    Future<List<R>> f = Future.succeededFuture(new ArrayList<>());
    for (T item : list) {
      f = f.compose(acc -> method.apply(item).map(r -> {
        acc.add(r);
        return acc;
      }));
    }
