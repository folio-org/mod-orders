...
import static org.folio.service.orders.utils.ProductIdUtils.isISBNValidationException;
...
public class PurchaseOrderLineService {
  ...
  public Future<Void> validateAndNormalizeISBNAndProductType(List<CompositePoLine> compositePoLines, RequestContext requestContext) {
    ...
    return inventoryCache.getInvalidISBNProductTypeId(requestContext)
      .compose(invalidIsbnTypeId -> inventoryCache.getISBNProductTypeId(requestContext)
        .compose(isbnTypeId -> {
          var setOfProductIds = buildSetOfProductIdsFromCompositePoLines(filteredCompLines, isbnTypeId);
          return org.folio.service.orders.utils.HelperUtils.executeWithSemaphores(setOfProductIds,
              productId -> inventoryCache.convertToISBN13(extractProductId(productId), requestContext)
                ...
                .recover(throwable -> isISBNValidationException(throwable) ?
            ...
            .map(productIdsMap -> {
              filteredCompLines.stream()
                .flatMap(poline -> poline.getDetails().getProductIds().stream())
                .filter(productId -> isISBN(isbnTypeId, productId))
                .forEach(productId -> Optional.ofNullable(productIdsMap.get(productId.getProductId())).ifPresentOrElse(
                    id -> productId.withQualifier(extractQualifier(productId.getProductId())).withProductId(id),
