...
public class TransactionsTotalFieldsPopulateService implements CompositeOrderDynamicDataPopulate {
    ...
    public CompletableFuture<CompositeOrderRetrieveHolder> populate(CompositeOrderRetrieveHolder holder, RequestContext requestContext) {
        return Optional.of(holder)
                ...
                .orElseGet(() -> CompletableFuture.completedFuture(holder.withTotalExpended(0d).withTotalEncumbered(0d)));
    }

    private CompletableFuture<CompositeOrderRetrieveHolder> withTotalFields(CompositeOrderRetrieveHolder holder, RequestContext requestContext) {
        return getCurrentEncumbrances(holder, requestContext)
    ...
    public CompletableFuture<List<Transaction>> getCurrentEncumbrances(CompositeOrderRetrieveHolder holder, RequestContext requestContext) {
