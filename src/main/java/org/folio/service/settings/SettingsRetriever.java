...
import static org.folio.service.settings.util.SettingFields.COLLECTION_SETTINGS;
...
public class SettingsRetriever {
  ...
  private CompletableFuture<Optional<Setting>> getSettingByKeyFromRemote(SettingKey settingKey, RequestContext requestContext) {
    RequestEntry requestEntry = new RequestEntry(SETTINGS_ENDPOINT)
      .withQuery(String.format(SETTINGS_BY_KEY_QUERY, settingKey.getName()))
      .withLimit(1)
      .withOffset(0);
    return restClient.getAsJsonObject(requestEntry, requestContext)
      .map(settings -> {
        var totalRecords = settings.getInteger(COLLECTION_TOTAL.getValue());
        var settingsArray = settings.getJsonArray(COLLECTION_SETTINGS.getValue());
        if (totalRecords == null || totalRecords != 1 || settingsArray == null || settingsArray.isEmpty()) {
          return Optional.<Setting>empty();
        }
        return Optional.of(settingsArray.getJsonObject(0).mapTo(Setting.class));
      })
      .toCompletionStage()
      .toCompletableFuture();
  }
