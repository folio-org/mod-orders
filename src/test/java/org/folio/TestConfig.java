...
public final class TestConfig {
  ...
  public static final Header X_OKAPI_URL =
      new Header(TestConstants.OKAPI_URL, "http://localhost:" + mockPort);
  ...
  public static final DockerImageName KAFKA_IMAGE_NAME =
  ...
  public static void deployVerticle()
    ...
    vertx
        .deployVerticle(RestVerticle.class.getName(), opt)
        .onComplete(
            res -> {
              if (res.succeeded()) {
  ...
  // Suppress "use try-with-resource" suggestion because in both places it is closed correctly in a
  // deferred fashion
  @SuppressWarnings("resource")
  public static KafkaContainer getKafkaContainer() {
    return new KafkaContainer(KAFKA_IMAGE_NAME).withStartupAttempts(20);
  }
  ...
  public static Context getFirstContextFromVertx(Vertx vertx) {
    return vertx.deploymentIDs().stream()
        .flatMap(
            id ->
                ((VertxImpl) vertx)
                    .deploymentManager().deployment(id).deployment().instances().stream())
