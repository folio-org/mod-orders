...
public class CacheIT {
  ...
  private final AcquisitionsUnitsService acquisitionsUnitsService =
      new AcquisitionsUnitsService(new RestClient());
  private final AcquisitionMethodsService acquisitionMethodsService =
      new AcquisitionMethodsService(new RestClient(), null);
  ...
  public void setUp() {
    ...
    mappingParametersCache =
        new MappingParametersCache(
            vertx,
            new RestClient(),
            acquisitionsUnitsService,
            acquisitionMethodsService,
            SETTINGS_LIMIT_PER_REQUEST,
            CACHE_EXPIRATION_TIME_SEC);
        ...
        .onComplete(
            ar -> {
              ...
              context.assertEquals(
                  result.orElse(new ProfileSnapshotWrapper()).getContentType(), JOB_PROFILE);
              async.complete();
            });
  }
  ...
  public void getMappingParametersFromCache(TestContext context) {
    ...
    String addressQueryParam =
        "query=" + encodeQuery("(module==TENANT and configName==tenant.addresses)");
                ...
                okJson(
                    new JsonObject()
                        .put(
                            "configs",
                            JsonArray.of(
                                new JsonObject().put("id", uuid).put("value", address.encode())))
                        .put("totalRecords", 10)
                        .toString())));
        ...
        .onComplete(
            ar -> {
              ...
              result
                  ...
                  .forEach(cnt -> context.assertEquals(uuid, cnt.getId()));
              ...
              result
                  ...
                  .forEach(units -> context.assertEquals(uuid, units.getId()));
              result
                  .getAcquisitionMethods()
                  .forEach(method -> context.assertEquals(uuid, method.getId()));
              result
                  .getTenantConfigurationAddresses()
                  .forEach(
                      a -> {
  ...
  public void getMappingParametersFromCacheWithError(TestContext context) {
    ...
    String acqUnitsQueryParam = "&query=" + encodeQuery("isDeleted==false");
    WireMock.stubFor(
        get("/organizations/organizations?limit=5000" + queryParam).willReturn(serverError()));
    WireMock.stubFor(get("/locations?limit=5000").willReturn(serverError()));

    WireMock.stubFor(get("/material-types?limit=5000").willReturn(serverError()));

    WireMock.stubFor(get("/identifier-types?limit=5000").willReturn(serverError()));

    WireMock.stubFor(get("/contributor-name-types?limit=5000").willReturn(serverError()));

    WireMock.stubFor(get("/finance/expense-classes?limit=5000").willReturn(serverError()));

    WireMock.stubFor(get("/finance/funds?limit=5000").willReturn(serverError()));

    WireMock.stubFor(
        get("/orders-storage/acquisition-methods?offset=0&limit=5000").willReturn(serverError()));

    WireMock.stubFor(
        get("/acquisitions-units-storage/units?offset=0" + acqUnitsQueryParam + "&limit=5000")
            .willReturn(serverError()));

    WireMock.stubFor(get("/configurations/entries?" + addressQueryParam).willReturn(serverError()));
    mappingParametersCache
        .get(okapiConnectionParams)
        .onComplete(
            ar -> {
