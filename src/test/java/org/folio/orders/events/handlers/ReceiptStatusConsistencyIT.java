...
public class ReceiptStatusConsistencyIT {
  ...
  private static final String PO_LINE_ID_TIED_TO_PIECE_WHEN_TOTAL_PIECES_EMPTY =
      "0dd8f1d2-ac2e-4155-a407-72071f6d5f4a";
  private static final String POLINE_UUID_TIED_TO_PIECE = "d471d766-8dbb-4609-999a-02681dea6c22";
  private static final String POLINE_UUID_TIED_TO_PIECE_PARTIALLY_RECEIVED =
      "fe47e95d-24e9-4a9a-9dc0-bcba64b51f56";
  private static final String POLINE_UUID_TIED_TO_PIECE_FULLY_RECEIVED =
      "2f443dbe-b3f6-417a-a4ec-db2623920b4a";
  ...
  void setUp() {
    ...
    vertx
        ...
        .consumer(
            TEST_ADDRESS,
            new ReceiptStatusConsistency(vertx, pieceStorageService, purchaseOrderLineService));
  }
  ...
  @Test
  void testSuccessReceiptStatusWhenReceiptStatusBetweenPiecesAndPoLineNotConsistent(
      VertxTestContext context) throws Throwable {
    logger.info("=== Test case when piece receipt status changes from Received to Expected ===");

    sendEvent(
        createBody(POLINE_UUID_TIED_TO_PIECE),
        context.succeeding(
            result -> {
  ...
  @Test
  void testSuccessPartiallyReceivedStatusWhenAtleastOneSuccessfullyReceivedPiece(
      VertxTestContext context) throws Throwable {
    logger.info(
        "=== Test case to verify partially received status when at least one successfully received piece ===");
    ...
    sendEvent(
        createBody(POLINE_UUID_TIED_TO_PIECE_PARTIALLY_RECEIVED),
        context.succeeding(
            result -> {
              logger.info("getPoLineSearches()--->" + getPoLineSearches());
              logger.info("getPoLineUpdates()--->" + getPoLineUpdates());
  ...
  @Test
  void testSuccessFullyReceivedStatusWhenAllPiecesSuccessfullyReceived(VertxTestContext context)
      throws Throwable {
    logger.info(
        "=== Test case to verify fully received status when all pieces successfully received ===");

    PoLine poLine1 =
        getMockAsJson(PO_LINES_MOCK_DATA_PATH, POLINE_UUID_TIED_TO_PIECE_FULLY_RECEIVED)
            .mapTo(PoLine.class);
    MockServer.addMockOrderData(Collections.singletonList(poLine1));

    sendEvent(
        createBody(POLINE_UUID_TIED_TO_PIECE_FULLY_RECEIVED),
        context.succeeding(
            result -> {
              logger.info("getPoLineSearches()--->" + getPoLineSearches());
              logger.info("getPoLineUpdates()--->" + getPoLineUpdates());
  ...
  void testSuccessReceiptStatusWhenTotalPiecesEmpty(VertxTestContext context) throws Throwable {
    ...
    sendEvent(
        createBody(PO_LINE_ID_TIED_TO_PIECE_WHEN_TOTAL_PIECES_EMPTY),
        context.succeeding(
            result -> {
              logger.info("getPoLineSearches()--->" + getPoLineSearches());
              logger.info("getPoLineUpdates()--->" + getPoLineUpdates());
  ...
  @Test
  void testPieceReceiptStatusFailureWhenNoMatchingPoLineForPiece(VertxTestContext context)
      throws Throwable {
    logger.info(
        "=== Test case when no poLines exists referenced by a piece which should throw a 404 Exception ===");

    sendEvent(
        createBody(BAD_PO_LINE_404),
        context.failing(
            result -> {
              logger.info("getPoLineSearches()--->" + getPoLineSearches());
              logger.info("getPoLineUpdates()--->" + getPoLineUpdates());
