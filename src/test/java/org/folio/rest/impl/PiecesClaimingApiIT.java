...
public class PiecesClaimingApiIT {
  ...
  private static Stream<Arguments> testPostPiecesClaimArgs() {
    ...
    return Stream.of(
        Arguments.of(
            "One piece One vendor One Job",
            0,
            17,
            69,
            mockHitDto,
            payloadFile,
            EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10_CLAIMS,
            SUCCESS,
            CREATED),
        Arguments.of(
            "One piece One vendor No Job",
            0,
            17,
            69,
            null,
  ...
  static class MockHitDto {
    ...
    public MockHitDto(
        int pieceSearches,
        int polSearches,
        int purchaseOrderRetrievals,
        int organizationSearches,
        int pieceUpdates,
        int claimingResults,
        int jobCreations) {
  ...
  void testPostPiecesClaim(
      ...
      String payloadFile,
      Header header,
      ClaimingPieceResult.Status expectedStatus,
    ...
    var poLine =
        getMockAsJson(PO_LINES_COLLECTION)
            ...
            .mapTo(PoLine.class);
    var purchaseOrder = getMinimalOrder(poLine).withVendor(organization.getId());
    var piece =
    ...
    var pieceSearches =
        getPieceSearches().stream()
            .map(JsonObject::mapFrom)
            .map(json -> json.mapTo(PieceCollection.class))
            .map(
                collection ->
                    collection.getPieces().stream()
                        .filter(entry -> Objects.nonNull(entry.getId()))
                        .filter(entry -> entry.getId().equals(piece.getId()))
                        .toList())
            .flatMap(Collection::stream)
            .toList();
    var polSearches =
        getPoLineSearches().stream()
            .map(JsonObject::mapFrom)
            .map(json -> json.getString(ENTRY_ID))
            .filter(Objects::nonNull)
            .filter(poLineId -> poLineId.equals(poLine.getId()))
            .toList();
    var purchaseOrderRetrievals =
        getPurchaseOrderRetrievals().stream()
            .map(JsonObject::mapFrom)
            .map(json -> json.getString(ENTRY_ID))
            .filter(Objects::nonNull)
            .filter(poLineId -> poLineId.equals(purchaseOrder.getId()))
            .toList();
    ...
    if (Objects.nonNull(claimingResults)) {
      ...
      pieceUpdates.forEach(
          pieceUpdate -> log.info("Updated Piece: {}", pieceUpdate.encodePrettily()));
      ...
      var claimedPieceIds =
          jobCreations.stream()
              ...
              .map(
                  job ->
                      job.getJsonObject(EXPORT_TYPE_SPECIFIC_PARAMETERS.getValue())
                          .getJsonObject(VENDOR_EDI_ORDERS_EXPORT_CONFIG.getValue())
                          .getJsonArray(CLAIM_PIECE_IDS.getValue())
                          .size())
      ...
      assertThat(
          ...
          is("Unable to generate claims for AMAZ because no claim integrations exist"));
