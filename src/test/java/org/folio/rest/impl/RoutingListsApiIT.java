...
public class RoutingListsApiIT {
  ...
  private static final String TEMPLATE_PROCESSING_REQUEST_ENDPOINT =
      "orders/routing-lists/" + ROUTING_LIST_ID + "/template";

  private final JsonObject routingListJsonReqData =
      getMockAsJson(ROUTING_LISTS_MOCK_DATA_PATH + "routing-list.json");
  private static boolean runningOnOwn;

  @Autowired private RoutingListService routingListService;
  ...
  static void before() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    initSpringContext(RoutingListsApiIT.ContextConfiguration.class);
  }
  ...
  void beforeEach() {
    ...
    sampleRoutingList = routingListJsonReqData.mapTo(RoutingList.class).withPoLineId(PO_LINE_UUID);
  }
  ...
  void testPostRoutingList() {
    ...
    doReturn(succeededFuture(sampleRoutingList))
        .when(routingListService)
        .createRoutingList(any(RoutingList.class), any(RequestContext.class));

    verifyPostResponse(
        ROUTING_LISTS_ENDPOINT,
        JsonObject.mapFrom(sampleRoutingList).encode(),
        prepareHeaders(EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10, X_OKAPI_USER_ID_WITH_ACQ_UNITS),
        APPLICATION_JSON,
  ...
  void testPostRoutingListShouldFailForInvalidOrderType() {
    logger.info(
        "=== Test POST Routing List should fail because it's POL has invalid order type ===");
    ...
    doReturn(failedFuture(new HttpException(RestConstants.VALIDATION_ERROR, errorsExpected)))
        .when(routingListService)
        .createRoutingList(any(RoutingList.class), any(RequestContext.class));
    ...
    assertThat(
        ...
        equalTo(INVALID_ROUTING_LIST_FOR_PO_LINE_FORMAT.getDescription()));
  }

  @Test
  void testPostRoutingListShouldFailForLimitReached() {
    logger.info(
        "=== Test POST Routing List should fail because it's POL has reached limit of Routing Lists ===");
    ...
    assertThat(
        errors.get(0).getMessage(),
        equalTo(ROUTING_LIST_LIMIT_REACHED_FOR_PO_LINE.getDescription()));
  }
  ...
  void testPostRoutingListWithInvalidPoLineId() {
    ...
    assertThat(
        errors.get(0).getMessage(), equalTo(PO_LINE_NOT_FOUND_FOR_ROUTING_LIST.getDescription()));
  }
  ...
  void testGetRoutingLists() {
    ...
    var collection =
        new RoutingListCollection()
            .withRoutingLists(List.of(sampleRoutingList))
            .withTotalRecords(1);
    doReturn(succeededFuture(collection))
        .when(routingListService)
        .getRoutingLists(anyInt(), anyInt(), any(), any(RequestContext.class));

    final RoutingListCollection respCollection =
        verifySuccessGet(ROUTING_LISTS_ENDPOINT, RoutingListCollection.class);
  ...
  void testGetRoutingListById() {
    ...
    doReturn(succeededFuture(sampleRoutingList))
        .when(routingListService)
        .getRoutingList(eq(ROUTING_LIST_UUID), any(RequestContext.class));

    final RoutingList resp =
        verifySuccessGet(
            String.format(ROUTING_LISTS_ID_PATH, ROUTING_LIST_UUID), RoutingList.class);
  ...
  void testPutRoutingList() {
    ...
    doReturn(succeededFuture())
        .when(routingListService)
        .updateRoutingList(any(RoutingList.class), any(RequestContext.class));

    verifyPut(
        String.format(ROUTING_LISTS_ID_PATH, ROUTING_LIST_UUID),
        JsonObject.mapFrom(sampleRoutingList).encode(),
  ...
  void testDeleteRoutingListByIdTest() {
    ...
    doReturn(succeededFuture())
        .when(routingListService)
        .deleteRoutingList(eq(ROUTING_LIST_UUID), any(RequestContext.class));
  ...
  void testDeleteRoutingListByIdWithInvalidFormatTest() {
    logger.info("=== Test delete Routing List by id ===");
    doReturn(failedFuture(new HttpException(BAD_REQUEST, ErrorCodes.GENERIC_ERROR_CODE)))
        .when(routingListService)
        .deleteRoutingList(eq(ID_BAD_FORMAT), any(RequestContext.class));
  ...
  void testDeleteNotExistentRoutingListTest() {
    logger.info("=== Test delete Routing List by id ===");
    doReturn(failedFuture(new HttpException(NOT_FOUND, ErrorCodes.GENERIC_ERROR_CODE)))
        .when(routingListService)
        .deleteRoutingList(eq(ID_DOES_NOT_EXIST), any(RequestContext.class));

    verifyDeleteResponse(
        String.format(ROUTING_LISTS_ID_PATH, ID_DOES_NOT_EXIST), APPLICATION_JSON, 404);
  }
  ...
  void testProcessTemplateRequest() {
    ...
    doReturn(succeededFuture(new JsonObject()))
        .when(routingListService)
        .processTemplateRequest(eq(ROUTING_LIST_ID), any(RequestContext.class));

    verifyGet(
        TEMPLATE_PROCESSING_REQUEST_ENDPOINT,
        prepareHeaders(X_OKAPI_URL, EMPTY_CONFIG_X_OKAPI_TENANT),
        APPLICATION_JSON,
        200);

    verify(routingListService, times(1))
        .processTemplateRequest(eq(ROUTING_LIST_ID), any(RequestContext.class));
  }
