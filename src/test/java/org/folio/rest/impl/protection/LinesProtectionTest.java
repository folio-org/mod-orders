...
public class LinesProtectionTest extends ProtectedEntityTestBase {
  ...
  public void testOperationWithNonExistedUnits(ProtectedOperations operation) {
    logger.info("=== Test corresponding order doesn't have units - expecting of call only to Assignments API ===");

    // Composite PO already contains acquisition unit IDs
    final Headers headers = prepareHeaders(X_OKAPI_URL, EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10, X_OKAPI_USER_ID);
  ...
  public void testOperationWithAllowedUnits(ProtectedOperations operation) {
    ...
    final Headers headers = prepareHeaders(X_OKAPI_URL, EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10, X_OKAPI_USER_ID);
    operation.process(LINES_PATH, encodePrettily(preparePoLine(NOT_PROTECTED_UNITS)), headers, APPLICATION_JSON, operation.getCode());
    // Verify number of sub-requests
    validateNumberOfRequests(1, 0);
  }
  ...
  public void testWithRestrictedUnitsAndAllowedUser(ProtectedOperations operation) {
    ...
    operation.process(LINES_PATH, encodePrettily(preparePoLine(PROTECTED_UNITS)),
      prepareHeaders(X_OKAPI_URL, EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10, X_OKAPI_USER_WITH_UNITS_ASSIGNED_TO_ORDER), APPLICATION_JSON, operation.getCode());
    // Verify number of sub-requests
  ...
  public void testWithProtectedUnitsAndForbiddenUser(ProtectedOperations operation) {
    ...
    final Headers headers = prepareHeaders(X_OKAPI_URL, EXIST_CONFIG_X_OKAPI_TENANT_LIMIT_10, X_OKAPI_USER_WITH_UNITS_NOT_ASSIGNED_TO_ORDER);
