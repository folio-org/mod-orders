...
import static org.folio.rest.impl.PieceApiIT.PIECES_ENDPOINT;
...
public class PiecesProtectionIT extends ProtectedEntityTestBase {
  ...
  void testOperationWithNonExistedUnits(ProtectedOperations operation) {
    logger.info(
        "=== Test corresponding order contains non-existent unit ids - expecting of call only to Units API ===");
    ...
    Errors errors =
        operation
            .process(
                PIECES_ENDPOINT,
                encodePrettily(preparePiece(NON_EXISTENT_UNITS)),
                headers,
                APPLICATION_JSON,
  ...
  void testOperationWithAllowedUnits(ProtectedOperations operation) {
    ...
    operation.process(
        PIECES_ENDPOINT,
        encodePrettily(preparePiece(NOT_PROTECTED_UNITS)),
  ...
  void testWithRestrictedUnitsAndAllowedUser(ProtectedOperations operation) {
    ...
    operation.process(
        PIECES_ENDPOINT,
        encodePrettily(preparePiece(PROTECTED_UNITS)),
        headers,
        operation.getContentType(),
        operation.getCode());
  ...
  void testWithProtectedUnitsAndForbiddenUser(ProtectedOperations operation) {
    ...
    assertThat(errors.getErrors(), hasSize(1));
    assertThat(
        errors.getErrors().getFirst().getCode(), equalTo(USER_NOT_A_MEMBER_OF_THE_ACQ.getCode()));
