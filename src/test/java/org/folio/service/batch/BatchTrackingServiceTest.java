...
public class BatchTrackingServiceTest {
  ...
  @InjectMocks private BatchTrackingService batchTrackingService;
  ...
  void shouldCreateBatchTrackingRecord(VertxTestContext vertxTestContext) {
    ...
    int totalRecords = 10;
    BatchTracking createdBatchTracking =
        new BatchTracking().withId(batchId).withTotalRecords(totalRecords);

    doReturn(succeededFuture(createdBatchTracking))
        .when(restClient)
        .post(
            any(RequestEntry.class),
            any(BatchTracking.class),
            eq(BatchTracking.class),
            eq(requestContext));

    // When
    Future<Void> future =
        batchTrackingService.createBatchTrackingRecord(batchId, totalRecords, requestContext);
