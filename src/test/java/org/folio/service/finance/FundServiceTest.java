...
public class FundServiceTest {
  ...
  void testShouldRetrieveFundById(VertxTestContext vertxTestContext) {
    ...
    CompositeFund compositeFund =
        new CompositeFund().withFund(new Fund().withId(fundId).withLedgerId(ledgerId));

    doReturn(succeededFuture(compositeFund))
        .when(restClient)
        .get(any(RequestEntry.class), eq(CompositeFund.class), eq(requestContext));
    // When
    ...
    vertxTestContext
        ...
        .onComplete(
            result -> {
              ...
              verify(restClient)
                  .get(argumentCaptor.capture(), eq(CompositeFund.class), eq(requestContext));
  ...
  void testShouldRetrieveFundsByLedgerId(VertxTestContext vertxTestContext) {
    ...
    doReturn(succeededFuture(fundCollection))
        .when(restClient)
        .get(any(RequestEntry.class), eq(FundCollection.class), eq(requestContext));
    // When
        ...
        .onComplete(
            result -> {
              ...
              ArgumentCaptor<RequestEntry> argumentCaptor =
                  ArgumentCaptor.forClass(RequestEntry.class);
              verify(restClient)
                  .get(argumentCaptor.capture(), eq(FundCollection.class), eq(requestContext));
              RequestEntry requestEntry = argumentCaptor.getValue();
  ...
  @Test
  void testShouldRetrieveFundsByFundIds(VertxTestContext vertxTestContext)
    ...
    String query =
        URLEncoder.encode("id==(" + fundId1 + " or " + fundId2 + ")", StandardCharsets.UTF_8);
    doReturn(succeededFuture(fundCollection))
        .when(restClient)
  ...
  void testShouldThrowHttpExceptionAsCauseIfFundNotFound(VertxTestContext vertxTestContext) {
    ...
    Error expError =
        new Error()
            ...
            .withMessage(String.format(FUNDS_NOT_FOUND.getDescription(), fundId));
    doReturn(failedFuture(new HttpException(404, expError)))
        .when(restClient)
        .get(any(RequestEntry.class), eq(CompositeFund.class), eq(requestContext));
    // When
    var future = fundService.retrieveFundById(fundId, requestContext);
    vertxTestContext
        ...
        .onComplete(
            thrown -> {
              ...
              assertEquals(
                  actError.getMessage(), String.format(FUNDS_NOT_FOUND.getDescription(), fundId));
              // Then
              vertxTestContext.completeNow();
            });
  }
  ...
  @Test
  void shouldThrowHttpExceptionAsCauseIfFundNotFoundWithGetAllFunds(
  ...
  void testShouldThrowNotHttpExceptionIfFundNotFound(VertxTestContext vertxTestContext) {
    // Given
    String fundId = UUID.randomUUID().toString();
    doReturn(failedFuture(new RuntimeException()))
        .when(restClient)
        .get(any(RequestEntry.class), eq(CompositeFund.class), eq(requestContext));
    // When
    var future = fundService.retrieveFundById(fundId, requestContext);
