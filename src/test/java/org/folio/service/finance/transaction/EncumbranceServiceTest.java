...
public class EncumbranceServiceTest {
  ...
  void testShouldInvokeBatchUpdateOnce() {
    ...
    doReturn(succeededFuture(null))
        ...
        .batchAllOrNothing(anyList(), anyList(), anyList(), anyList(), eq(requestContextMock));
    // When
    encumbranceService.updateEncumbrances(
        Arrays.asList(encumbrance, encumbrance), requestContextMock);
    // Then
  ...
  void test() {
    ...
    doReturn(succeededFuture(fiscalYear))
        ...
        .getCurrentFiscalYearByFundId(anyString(), eq(requestContextMock));
    doReturn(Future.succeededFuture(transactions))
        .when(transactionService)
        .getTransactions(anyString(), eq(requestContextMock));

    // When
    Future<List<Transaction>> result =
        encumbranceService.getPoLineReleasedEncumbrances(poLine, requestContextMock);
  ...
  void shouldSimplyDeleteWhenDeleteOrderEncumbrances() {
    ...
    doReturn(Future.succeededFuture(null))
        .when(transactionService)
        .batchReleaseAndDelete(anyList(), eq(requestContextMock));
  ...
  void shouldSimplyDeleteWhenDeletePoLineEncumbrances() {
    ...
    Transaction encumbrance =
        new Transaction()
            .withId(UUID.randomUUID().toString())
            .withEncumbrance(
                new Encumbrance().withSourcePurchaseOrderId(orderId).withSourcePoLineId(lineId));
    Transaction encumbrance2 =
        new Transaction()
            .withId(UUID.randomUUID().toString())
            .withEncumbrance(
  ...
  void shouldCallRemoveEncumbranceLinks() {
    ...
    Transaction encumbrance1 =
        new Transaction()
            ...
            .withEncumbrance(
                new Encumbrance().withSourcePurchaseOrderId(orderId).withSourcePoLineId(poLineId1));
    Transaction encumbrance2 =
        new Transaction()
            .withId(encumbrance2Id)
            .withEncumbrance(
                new Encumbrance().withSourcePurchaseOrderId(orderId).withSourcePoLineId(poLineId1));
    Transaction encumbrance3 =
        new Transaction()
            .withId(encumbrance3Id)
            .withEncumbrance(
                new Encumbrance().withSourcePurchaseOrderId(orderId).withSourcePoLineId(poLineId2));
                ...
                List.of(
                    new Adjustment()
                        .withFundDistributions(
                            List.of(
                                new org.folio.rest.acq.model.invoice.FundDistribution()
                                    .withEncumbrance(encumbrance3Id)))));
    ...
    // When
    Future<Void> result =
        encumbranceService.deleteEncumbranceLinksInInvoiceLines(transactions, requestContextMock);
    assertTrue(result.succeeded());
    result.result();

    // Then
    verify(invoiceLineService, times(1))
        .removeEncumbranceLinks(
  ...
  void shouldNotCallRemoveEncumbranceLinks() {
    ...
    Transaction encumbrance1 =
        new Transaction()
            .withId(encumbrance1Id)
            .withEncumbrance(
                new Encumbrance().withSourcePurchaseOrderId(orderId).withSourcePoLineId(poLineId));
    Transaction encumbrance2 =
        new Transaction()
            .withId(encumbrance2Id)
            .withEncumbrance(
