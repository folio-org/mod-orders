...
public class OpenToClosedEncumbranceStrategyTest {
  ...
  @InjectMocks private OpenToClosedEncumbranceStrategy openToClosedEncumbranceStrategy;
  ...
  void testShouldReleaseAndSetOrderStatusToEncumbrances() {
    ...
    doReturn(succeededFuture(encumbrances))
        ...
        .getEncumbrancesByPoLinesFromCurrentFy(any(), any());
    doReturn(succeededFuture(null)).when(encumbranceService).updateEncumbrances(any(), any());

    // When
    Future<Void> result =
        openToClosedEncumbranceStrategy.processEncumbrances(
    ...
    verify(encumbranceService, times(1))
        .updateEncumbrances(
            argThat(
                transactions ->
                    transactions.size() == 1
                        && transactions.get(0).getEncumbrance().getOrderStatus()
                            == Encumbrance.OrderStatus.CLOSED
                        && transactions.get(0).getEncumbrance().getStatus()
