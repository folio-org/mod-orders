...
public class InventoryInstanceManagerTest {
  ...
  @Mock private InventoryCache inventoryCache;
  @Mock private SharingInstanceService sharingInstanceService;
  ...
  void shouldUseExistingInstanceId() {
    ...
    inventoryInstanceManager
        .openOrderHandleInstance(poLine, false, new HashSet<>(), requestContext)
        .result();
  ...
  void shouldCreateInstanceRecordWithSuppressDiscoveryWhenPoLineSuppressInstanceFromDiscoveryIsTrue(
    ...
    try (var mockedUtils = Mockito.mockStatic(InventoryUtils.class)) {
      ...
      mockedUtils
          .when(
              () ->
                  InventoryUtils.getEntryId(
                      eq(commonSettingsCache),
                      eq(inventoryCache),
                      eq(INSTANCE_TYPES),
                      eq(MISSING_INSTANCE_TYPE),
                      eq(requestContext)))
          ...
          .when(
              () ->
                  InventoryUtils.getEntryId(
                      eq(commonSettingsCache),
                      eq(inventoryCache),
                      eq(INSTANCE_STATUSES),
                      eq(MISSING_INSTANCE_STATUS),
                      eq(requestContext)))
      ...
      when(restClient.postJsonObjectAndGetId(
      ...
      verify(restClient)
          .postJsonObjectAndGetId(
              any(RequestEntry.class), instanceCaptor.capture(), any(RequestContext.class));
  ...
  void shouldNotShareInstanceAmongTenantsWhenInstancesAlreadyShared() {
    ...
    Optional<ConsortiumConfiguration> configuration =
        Optional.of(new ConsortiumConfiguration(CENTRAL_TENANT, UUID.randomUUID().toString()));
    ...
    doReturn(succeededFuture(collection))
        .when(sharingInstanceService)
        .getSharingInstances(instanceId, configuration.get(), requestContext);

    inventoryInstanceManager
        .openOrderHandleInstance(poLine, false, new HashSet<>(), requestContext)
        .result();

    verify(sharingInstanceService, never())
        .createShadowInstance(
            anyString(), any(ConsortiumConfiguration.class), any(RequestContext.class));
  }
  ...
  void shouldShareInstanceWhenFoundInCentralTenant() {
    ...
    doReturn(succeededFuture(null))
        .when(restClient)
        .getAsJsonObject(any(RequestEntry.class), eq(true), any(RequestContext.class));

    SharingInstanceCollection collection = new SharingInstanceCollection();
    SharingInstance sharingInstance =
        new SharingInstance(UUID.fromString(centralInstanceId), MEMBER_TENANT_2, CENTRAL_TENANT);
    collection.setSharingInstances(List.of(sharingInstance));

    doReturn(succeededFuture(collection))
        .when(sharingInstanceService)
        .getSharingInstances(centralInstanceId, configuration.get(), requestContext);
    ...
    verify(sharingInstanceService, times(1))
        .createShadowInstance(
            eq(centralInstanceId), eq(configuration.get()), requestContextCaptor.capture());
  ...
  private PoLine getPoLine(String instanceId, List<Location> locations) {
    ...
    List<ProductId> productIds =
        List.of(
            new ProductId()
                .withProductId("10407849")
                .withProductIdType("913300b2-03ed-469a-8179-c1092c991227"));
  ...
  private SharingInstanceCollection getSharingInstanceCollection(String instanceId) {
    ...
    SharingInstance sharingInstance =
        new SharingInstance(UUID.fromString(instanceId), MEMBER_TENANT_1, CENTRAL_TENANT);
