...
public class CompositeOrderTotalFieldsPopulateServiceTest {
  ...
  @InjectMocks private CompositeOrderTotalFieldsPopulateService populateService;
  ...
  void shouldPopulateTotalFieldsWhenInvoicesInvoiceLinesAndTransactionsExist() {
    ...
    CompositeOrderRetrieveHolder holder =
        new CompositeOrderRetrieveHolder(order).withFiscalYearId(fiscalYearId);

    when(invoiceService.getInvoicesByOrderId(anyString(), any()))
        .thenReturn(Future.succeededFuture(invoices));
    when(invoiceLineService.getInvoiceLinesByInvoiceIdAndStatus(
            eq(invoice1.getId()), eq(PAID), any()))
        .thenReturn(Future.succeededFuture(List.of(invoiceLine1)));
    when(invoiceLineService.getInvoiceLinesByInvoiceIdAndStatus(
            eq(invoice2.getId()), eq(PAID), any()))
    ...
    CompositeOrderRetrieveHolder resultHolder =
  ...
  void shouldReturnZeroWhenNoInvoicesAndTransactionsExist() {
    ...
    CompositeOrderRetrieveHolder holder =
        new CompositeOrderRetrieveHolder(order).withFiscalYearId(UUID.randomUUID().toString());
  ...
  void shouldReturnZeroWhenNoInvoiceLinesAndTransactionsExist() {
    ...
    when(invoiceService.getInvoicesByOrderId(anyString(), any()))
        .thenReturn(Future.succeededFuture(invoices));
    when(invoiceLineService.getInvoiceLinesByInvoiceIdAndStatus(anyString(), eq(PAID), any()))
        .thenReturn(Future.succeededFuture(Collections.emptyList()));
    when(transactionService.getTransactions(anyString(), any()))
        .thenReturn(Future.succeededFuture(Collections.emptyList()));
  ...
  void shouldConvertEncumbranceCurrencyToSystemCurrency() {
    ...
    PoLine poLine = new PoLine().withId(UUID.randomUUID().toString());
    Transaction transaction =
        new Transaction().withTransactionType(ENCUMBRANCE).withAmount(100d).withCurrency("EUR");
    CompositePurchaseOrder order =
        new CompositePurchaseOrder()
            .withId(UUID.randomUUID().toString())
            .withPoLines(List.of(poLine));
