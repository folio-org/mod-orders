...
public class PurchaseOrderStorageServiceTest {
  @InjectMocks private PurchaseOrderStorageService purchaseOrderStorageService;
  ...
  void successRetrievePurchaseOrdersByQuery() {
    ...
    List<PurchaseOrder> purchaseOrders =
        Collections.singletonList(new PurchaseOrder().withId(orderId));

    PurchaseOrderCollection purchaseOrderCollection =
        new PurchaseOrderCollection().withPurchaseOrders(purchaseOrders).withTotalRecords(1);
    ...
    PurchaseOrderCollection actOrders =
        purchaseOrderStorageService
            .getPurchaseOrders(expectedQuery, 0, Integer.MAX_VALUE, requestContext)
            .result();

    verify(restClientMock)
        .get(any(RequestEntry.class), eq(PurchaseOrderCollection.class), eq(requestContext));
  ...
  void successRetrievePurchaseOrdersByIds(VertxTestContext vertxTestContext) {
    ...
    var future =
        purchaseOrderStorageService.getPurchaseOrdersByIds(List.of(orderId), requestContext);
    vertxTestContext
            ...
            result -> {
              verify(restClientMock)
                  .get(
                      ...
                      eq(PurchaseOrderCollection.class),
                      eq(requestContext));
