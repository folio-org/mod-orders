...
public class RoutingListValidatorTest {
  ...
  void testValidateRoutingList() {
    ...
    List<Error> errorList =
        RoutingListValidatorUtil.validateRoutingList(sampleRoutingList, collection, samplePoLine);
  ...
  void testValidateRoutingListWithPOLineLimitReached() {
    ...
    List<Error> errors =
        RoutingListValidatorUtil.validateRoutingList(distinctRoutingList, collection, samplePoLine);
    assertEquals(errors.size(), 1);
    assertEquals(
        errors.get(0).getMessage(), ROUTING_LIST_LIMIT_REACHED_FOR_PO_LINE.getDescription());
  }
  ...
  void testValidateRoutingListWithPOLineInvalidOrderFormat() {
    ...
    assertEquals(errors.size(), 1);
    assertEquals(
        errors.get(0).getMessage(), INVALID_ROUTING_LIST_FOR_PO_LINE_FORMAT.getDescription());
  }
