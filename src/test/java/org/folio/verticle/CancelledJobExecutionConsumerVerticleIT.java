...
public class CancelledJobExecutionConsumerVerticleIT {
  ...
  @Test
  public void shouldReadAndPutMultipleJobIdsToCache()
  ...
  public void shouldReadAllEventsFromTopicIfVerticleWasRestarted(TestContext context)
    ...
    await()
        .atMost(ofSeconds(3))
        .untilAsserted(
            () -> idsBatch1.forEach(id -> assertTrue(cancelledJobsIdsCache.contains(id))));
    ...
    await()
        .atMost(ofSeconds(3))
        .untilAsserted(
            () -> idsBatch2.forEach(id -> assertTrue(cancelledJobsIdsCache.contains(id))));
  }
  ...
  private Future<String> deployVerticle(CancelledJobsIdsCache cancelledJobsIdsCache) {
    DeploymentOptions deploymentOptions =
        new DeploymentOptions().setThreadingModel(ThreadingModel.WORKER).setInstances(1);

    return vertx
        .deployVerticle(
            () ->
                new CancelledJobExecutionConsumerVerticle(cancelledJobsIdsCache, kafkaConfig, 1000),
            deploymentOptions)
  ...
  private KafkaProducer<String, String> createKafkaProducer() {
    ...
    producerProperties.setProperty(
        BOOTSTRAP_SERVERS_CONFIG, TestConfig.kafkaContainer.getBootstrapServers());
  ...
  private String formatToKafkaTopicName(String eventType) {
    return KafkaTopicNameHelper.formatTopicName(
        KAFKA_ENV, getDefaultNameSpace(), TENANT_ID, eventType);
  }
}
